 ...
 * @property {int} countE1 - элемент на странице для вывода  нформации о счете
 * @type {{count: null, countE1: null, init(): void, drop(): void, increment(): void, render(): void}}
 */
const score = {
    settings,
  count: null,
    countE1: null,
    /**
     * Инициализация счетчика очков игры
     */
    init(){
    ...
    /**
     * сброс счетчика очков игры
     */
    drop(){
        this.count = 0;
        this.countE1.textContent = this.count.toString();

    },
    /**
     * Инкремент счетчика очков игры
     */
    increment(){
      this.count += 1;
      if(this.count % 5 === 0){
 ...
 * @property {int} maxX Максимальная координата Х
 * @property {int} maxY Максимальная координата Y
 */
const snake = {
  body: null,
  direction: null,
  ...
  init(startBody, direction) {
    ...
    this.maxX = this.settings.colsCount - 1;
    this.maxY = this.settings.rowsCount - 1;
  ...
  getNextStepHeadPoint() {
    ...
    switch (this.direction) {
      case 'up':
        return firstPoint.y === 0 ? {x: firstPoint.x, y: this.maxY} : {x: firstPoint.x, y: firstPoint.y - 1};
      case 'right':
        return firstPoint.x === this.maxX ? {x: 0, y: firstPoint.y}
        : {x: firstPoint.x + 1, y: firstPoint.y};
      case 'down':
        return firstPoint.y === this.maxY ? {x: firstPoint.x, y: 0}
        : {x: firstPoint.x, y: firstPoint.y + 1};
      case 'left':
        return firstPoint.x === 0 ? {x: this.maxX, y: firstPoint.y}
        : {x: firstPoint.x - 1, y: firstPoint.y};
    }
  },
